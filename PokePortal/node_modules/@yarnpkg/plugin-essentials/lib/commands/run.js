...
class RunCommand extends cli_1.BaseCommand {
    constructor() {
        super(...arguments);
        this.inspect = clipanion_1.Option.String(`--inspect`, false, {
            tolerateBoolean: true,
            description: `Forwarded to the underlying Node process when executing a binary`,
        });
        this.inspectBrk = clipanion_1.Option.String(`--inspect-brk`, false, {
            tolerateBoolean: true,
            description: `Forwarded to the underlying Node process when executing a binary`,
        });
        ...
        this.scriptName = clipanion_1.Option.String();
        this.args = clipanion_1.Option.Proxy();
    }
    async execute() {
        const configuration = await core_1.Configuration.find(this.context.cwd, this.context.plugins);
        const { project, workspace, locator } = await core_1.Project.find(configuration, this.context.cwd);
        ...
        // for the given name
        if (!this.binariesOnly && await core_2.scriptUtils.hasPackageScript(effectiveLocator, this.scriptName, { project }))
            return await core_2.scriptUtils.executePackageScript(effectiveLocator, this.scriptName, this.args, { project, stdin: this.context.stdin, stdout: this.context.stdout, stderr: this.context.stderr });
        ...
        const binaries = await core_2.scriptUtils.getPackageAccessibleBinaries(effectiveLocator, { project });
        ...
        if (!this.topLevel && !this.binariesOnly && workspace && this.scriptName.includes(`:`)) {
            ...
            if (filteredWorkspaces.length === 1) {
                return await core_2.scriptUtils.executeWorkspaceScript(filteredWorkspaces[0], this.scriptName, this.args, { stdin: this.context.stdin, stdout: this.context.stdout, stderr: this.context.stderr });
            }
        }
        if (this.topLevel) {
            if (this.scriptName === `node-gyp`) {
                throw new clipanion_1.UsageError(`Couldn't find a script name "${this.scriptName}" in the top-level (used by ${core_2.structUtils.prettyLocator(configuration, locator)}). This typically happens because some package depends on "node-gyp" to build itself, but didn't list it in their dependencies. To fix that, please run "yarn add node-gyp" into your top-level workspace. You also can open an issue on the repository of the specified package to suggest them to use an optional peer dependency.`);
            }
            else {
                throw new clipanion_1.UsageError(`Couldn't find a script name "${this.scriptName}" in the top-level (used by ${core_2.structUtils.prettyLocator(configuration, locator)}).`);
            }
        }
        else {
            if (this.scriptName === `global`)
                throw new clipanion_1.UsageError(`The 'yarn global' commands have been removed in 2.x - consider using 'yarn dlx' or a third-party plugin instead`);
            const userCommand = [this.scriptName].concat(this.args);
            for (const [pluginName, candidates] of cli_1.pluginCommands)
                for (const candidate of candidates)
                    if (userCommand.length >= candidate.length && JSON.stringify(userCommand.slice(0, candidate.length)) === JSON.stringify(candidate))
                        throw new clipanion_1.UsageError(`Couldn't find a script named "${this.scriptName}", but a matching command can be found in the ${pluginName} plugin. You can install it with "yarn plugin import ${pluginName}".`);
            throw new clipanion_1.UsageError(`Couldn't find a script named "${this.scriptName}".`);
        }
    }
}
RunCommand.paths = [
